...
impl FeatureLockedError {
    pub fn to_response(&self) -> FeatureLockedErrorResponse {
        FeatureLockedErrorResponse {
            ...
            required_tier: PRO_TIER.to_string(),
            upgrade_hint_key: "upgrade.pro".to_string(),
        }
    }
}
...
pub async fn get_entitlements_snapshot(
    ...
    Ok(EntitlementsResponse {
        ...
        policy: EntitlementsPolicy {
            ai_is_pro_only: config.policies.ai_is_pro_only,
...
mod tests {
    ...
    #[test]
    fn pro_tier_inherits_free_and_has_pro_entitlements() {
        let Some(config) = config_or_skip() else {
            return;
        };
        let Some(entitlements) = resolve_or_skip(config, "pro") else {
            return;
        };

        assert!(entitlements.contains(&"core.discovery".to_string()));
        assert!(entitlements.contains(&"ai.copilot.weekly_grow_plan".to_string()));
        assert!(entitlements.contains(&"pro.analytics.read".to_string()));
    }
    ...
    fn policy_flags_match_phase6_rules() {
        ...
        assert!(config.policies.ai_is_pro_only);
        assert!(config.policies.free_reminders_are_deterministic_only);
    }

    #[test]
    fn pro_entitlement_denied_for_free_tier() {
        let Some(config) = config_or_skip() else {
            return;
        };
    ...
    #[test]
    fn pro_entitlement_allowed_for_pro_tier() {
        let Some(config) = config_or_skip() else {
            return;
        };

        let has_entitlement = tier_has_entitlement(config, "pro", "ai.copilot.weekly_grow_plan");
        assert!(has_entitlement.is_ok());
